	...
	tests := []struct {
	...
	}{
		{
			...
			input: UpdateItemInput{
				...
				Brand:         stringPtr("新ブランド"),
				PurchasePrice: intPtr(200000),
			},
			setupMock: func(mockRepo *MockItemRepository) {
				existingItem, _ := entity.NewItem("古い時計", "時計", "古いブランド", 100000, "2023-01-01")
				existingItem.ID = 1
				mockRepo.On("FindByID", mock.Anything, int64(1)).Return(existingItem, nil)

				updatedItem, _ := entity.NewItem("新しい時計", "時計", "新ブランド", 200000, "2023-01-01")
				updatedItem.ID = 1
		...
		},
		{
			...
			setupMock: func(mockRepo *MockItemRepository) {
				existingItem, _ := entity.NewItem("古い名前", "時計", "ROLEX", 100000, "2023-01-01")
				existingItem.ID = 1
				mockRepo.On("FindByID", mock.Anything, int64(1)).Return(existingItem, nil)

				updatedItem, _ := entity.NewItem("新しい名前", "時計", "ROLEX", 100000, "2023-01-01")
				updatedItem.ID = 1
				mockRepo.On("Update", mock.Anything, mock.AnythingOfType("*entity.Item")).Return(updatedItem, nil)
			},
			expectError: false,
		},
		{
			...
			id:   1,
			input: UpdateItemInput{
				Brand: stringPtr("セイコー"),
			},
			setupMock: func(mockRepo *MockItemRepository) {
				existingItem, _ := entity.NewItem("時計1", "時計", "ROLEX", 100000, "2023-01-01")
				existingItem.ID = 1
				mockRepo.On("FindByID", mock.Anything, int64(1)).Return(existingItem, nil)

				updatedItem, _ := entity.NewItem("時計1", "時計", "セイコー", 100000, "2023-01-01")
				updatedItem.ID = 1
				mockRepo.On("Update", mock.Anything, mock.AnythingOfType("*entity.Item")).Return(updatedItem, nil)
			},
			expectError: false,
		},
		{
			...
			setupMock: func(mockRepo *MockItemRepository) {
				...
				updatedItem, _ := entity.NewItem("時計1", "時計", "ROLEX", 50000, "2023-01-01")
				updatedItem.ID = 1
				mockRepo.On("Update", mock.Anything, mock.AnythingOfType("*entity.Item")).Return(updatedItem, nil)
			},
			expectError: false,
		...
		},
		{
			...
			setupMock: func(mockRepo *MockItemRepository) {
				...
				existingItem.ID = 1
				mockRepo.On("FindByID", mock.Anything, int64(1)).Return(existingItem, nil)
				// Updateは呼ばれない（バリデーションエラーで失敗）
			},
			expectError: true,
			expectedErr: domainErrors.ErrInvalidInput,
		},
		{
			name: "異常系: バリデーションエラー（purchase_priceが負の値）",
			id:   1,
			input: UpdateItemInput{
				PurchasePrice: intPtr(-1000),
			},
			setupMock: func(mockRepo *MockItemRepository) {
				existingItem, _ := entity.NewItem("時計1", "時計", "ROLEX", 100000, "2023-01-01")
				existingItem.ID = 1
